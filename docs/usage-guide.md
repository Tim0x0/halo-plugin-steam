...
富文本编辑器里插入的 **Steam 游戏卡片** 也做了同样处理：当某个 App ID 在 Steam 商店取不到详情（appdetails 返回 `success:false`）时，会先用 GetItems 的 `visible=false` 二次确认——确认不可见则显示「**该游戏不可用**（已下架、区域限制或 ID 无效）」提示（保留商店链接、不再显示重试）；若只是临时取不到（如接口限流），仍按「加载失败」处理并可重试。
