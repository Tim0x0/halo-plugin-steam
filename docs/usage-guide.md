...
| 游戏卡片语言 | 游戏卡片显示的语言和价格货币 | 自动（浏览器语言） |

**语言选项说明：**

- **自动（浏览器语言）**：根据每个访客的浏览器语言自动选择。例如中文浏览器看到简体中文描述和人民币价格，英文浏览器看到英文描述和美元价格。
- **指定语言**：选择具体语言后，所有访客看到相同的语言和对应地区的货币价格，忽略浏览器设置。
...
> 💡 修改语言设置后，需要刷新缓存或等待缓存过期后生效。不同语言的游戏数据会分别缓存。
...
<th:block th:with="gameList=${steamFinder.getOwnedGames(1, 12)}">
    <div th:if="${gameList != null}" th:each="game : ${gameList.items}">
        <img th:src="${game.headerImageUrl}" th:alt="${game.name}">
        <span th:text="${game.name}">游戏名</span>
        <span th:text="${game.playtimeForeverFormatted}">总游玩时长</span>
    </div>
    <!-- 分页信息 -->
